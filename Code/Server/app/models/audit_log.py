from sqlalchemy import Column, BigInteger, String, JSON, DateTime, ForeignKey, func
...
class AuditLog(Base):
    ...
    audit_log_id = Column(BigInteger, primary_key=True, autoincrement=True, comment="Mã nhật ký vết duy nhất")
    actor_id = Column(BigInteger, ForeignKey("USERS.user_id", ondelete="RESTRICT"), nullable=False, index=True, comment="Người thực hiện hành động")
    ...
    target_id = Column(BigInteger, nullable=True, comment="ID của bản ghi bị tác động")
